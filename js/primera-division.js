...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function init() {
        ...
        if (!window.CLUB_DATA || !CLUB_DATA.primeraDivisionHistorico) {
            ...
            jornadasContainer.innerHTML = '<p style="text-align:center; padding:40px; color:#666;">Error: No hay datos disponibles. Asegúrate de añadir la estructura de datos en data.js</p>';
            return;
        }

        // Cargar temporadas disponibles
        const temporadas = Object.keys(CLUB_DATA.primeraDivisionHistorico).sort().reverse();
        if (temporadas.length === 0) {
            jornadasContainer.innerHTML = '<p style="text-align:center; padding:40px; color:#666;">No hay temporadas registradas todavía.</p>';
            return;
        }
    ...
    function extraerRivales() {
        ...
        datosTemporada.jornadas.forEach(jornada => {
            jornada.partidos.forEach(partido => {
                ...
                if (partido.visitante !== 'Real Oviedo') todosLosRivales.add(partido.visitante);
            });
        });
        ...
        Array.from(todosLosRivales).sort().forEach(rival => {
    ...
    function aplicarFiltros() {
        ...
        datosTemporada.jornadas.forEach(jornada => {
            ...
            const partidosFiltrados = jornada.partidos.filter(partido => {
                ...
                if (soloOviedoChecked) {
                    const esOviedo = partido.local === 'Real Oviedo' || partido.visitante === 'Real Oviedo';
                    if (!esOviedo) return false;
                }

                // Filtro: Por rival
                if (rivalSeleccionado !== 'todos') {
                    const esContraRival = partido.local === rivalSeleccionado || partido.visitante === rivalSeleccionado;
                    if (!esContraRival) return false;
                }

                // Filtro: Por resultado (solo aplica si es partido del Oviedo)
                if (resultadoSeleccionado !== 'todos' && (partido.local === 'Real Oviedo' || partido.visitante === 'Real Oviedo')) {
                    const esLocal = partido.local === 'Real Oviedo';
                    const golesOviedo = esLocal ? partido.golesLocal : partido.golesVisitante;
                    const golesRival = esLocal ? partido.golesVisitante : partido.golesLocal;

                    if (resultadoSeleccionado === 'victoria' && golesOviedo <= golesRival) return false;
                    if (resultadoSeleccionado === 'empate' && golesOviedo !== golesRival) return false;
                    if (resultadoSeleccionado === 'derrota' && golesOviedo >= golesRival) return false;
                }
        ...
        noResultados.style.display = totalPartidosMostrados === 0 ? 'block' : 'none';
        
        // Actualizar contador
        if (totalPartidosMostrados > 0 || 
            soloOviedoChecked || 
    ...
    function renderPartido(partido) {
        ...
        // URLs de escudos (usar placeholder si no hay)
        const escudoLocal = partido.escudoLocal || 'https://via.placeholder.com/60';
        const escudoVisitante = partido.escudoVisitante || 'https://via.placeholder.com/60';
                ...
                <div class="resultado-box">
                    ...
                    <span class="estado-partido">
                        ${partido.estado === 'finalizado' ? 'Finalizado' : 
                          partido.estado === 'pendiente' ? 'Pendiente' : 'En juego'}
                    </span>
                </div>
    ...
    function setupEventListeners() {
        ...
        temporadaSelector.addEventListener('click', function(e) {
            if (e.target.classList.contains('temporada-tab')) {
                cambiarTemporada(e.target.dataset.temporada);
            }
        });
        ...
        btnColapsar.addEventListener('click', function() {
            document.querySelectorAll('.jornada-section').forEach(j => j.classList.add('collapsed'));
            this.style.display = 'none';
            btnExpandir.style.display = 'inline-flex';
        });

        btnExpandir.addEventListener('click', function() {
            document.querySelectorAll('.jornada-section').forEach(j => j.classList.remove('collapsed'));
            this.style.display = 'none';
    ...
    function updateToggleButtons() {
        const todasColapsadas = document.querySelectorAll('.jornada-section:not(.collapsed)').length === 0;
        if (todasColapsadas) {
    ...
    window.toggleJornada = function(header) {
        ...
        const todasColapsadas = document.querySelectorAll('.jornada-section:not(.collapsed)').length === 0;
        const todasExpandidas = document.querySelectorAll('.jornada-section.collapsed').length === 0;
